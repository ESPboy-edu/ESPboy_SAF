...
#define SCL_BOARD_STATE_SIZE 68
...
/**
  ...
  - After this more bytes follow to represent global state, these are:
    ...
    - 67: The last byte is always 0 to properly terminate the string in case
...
/**
  Initializes given chess 960 (Fisher random) position and disables castling
  (even though castling is possible in chess 960, this engine doesn't support
  it for this variant).
*/
...
typedef struct
{
  ...
  uint32_t prevMoves[16];     ///< stores last moves, for repetition detection
...
/**
  ...
  to NULL will assume traditional starting position. WARNING: if startState is
...
/**
  ...
  function is NULL, AI will always make the first best move it finds, if it is
  not NULL and randomness is 0, AI will randomly pick between the equally best
  moves, if it is not NULL and randomness is positive, AI will randomly choose
...
void SCL_boardUndoMove(SCL_Board board, SCL_MoveUndo moveUndo)
{
  ...
  else if (board[moveUndo.squareFrom] == 'k' && 
    moveUndo.squareFrom == 60) // black castling
  {
    ...
    moveUndo.squareFrom == 4) // white castling
  {
  ...
  char promotePiece)
{
  ...
  {
    // after rook move disables castling to one side

    uint8_t andVal = 0xff;

    switch (squareFrom)
    {
      case 0: andVal = (uint8_t) ~0x20; break;
      case 7: andVal = (uint8_t) ~0x10; break;
      case 56: andVal = (uint8_t) ~0x80; break;
      case 63: andVal = (uint8_t) ~0x40; break;
      default: break;
    }

    board[SCL_BOARD_ENPASSANT_CASTLE_BYTE] &= andVal;
  }
...
int16_t SCL_boardEvaluateStatic(SCL_Board board)
{
  ...
  switch (position)
  {
    ...
    /*
      ...
      - ATTACK_BONUS pawn for any attacked piece
...
int16_t _SCL_currentPositionScore;
  ...
  int16_t alphaBeta, int8_t takenSquare)
{
  ...
  int16_t eval = _SCL_staticEvaluationFunction(board) * valueMultiply;
...
#if SCL_DEBUG_AI
  uint8_t bestSquareFrom = 0, bestSquareTo = 0;
#endif
      ...
      if (s != '.' && SCL_pieceIsWhite(s) == whitesTurn)
      {
        ...
        if (!SCL_squareSetEmpty(moves))
        {
          ...
          SCL_SQUARE_SET_ITERATE_BEGIN(moves)
            ...
            if (board[iteratedSquare] != '.' &&
              (!extended || (extended && iteratedSquare == takenSquare)))
...
#if SCL_DEBUG_AI
  if (bestSquareTo != 0 || bestSquareTo != 0)
    printf("  (level %d) %d -> %d: %d\n",depth,bestSquareFrom,bestSquareTo,bestMoveValue * valueMultiply);
#endif
  ...
  for (uint8_t i = 0; i < SCL_BOARD_SQUARES; ++i)
      ...
      SCL_boardWhitesTurn(board) == SCL_pieceIsWhite(board[i]))
    {
      ...
      SCL_SQUARE_SET_ITERATE_BEGIN(moves)
        ...
        int16_t score = SCL_boardEvaluateDynamic(board,baseDepth - 1,
          extensionExtraDepth,evalFunc);

#if SCL_DEBUG_AI
        printf("%d -> %d: %d\n",i,iteratedSquare,score);
#endif
  ...
  char promoteTo)
{
  ...
  game->prevMoves[14] = squareFrom;
  game->prevMoves[15] = squareTo;
  ...
  if (game->ply >= 8 &&
    ...
    game->prevMoves[2] == game->prevMoves[15] &&

    game->prevMoves[3] == game->prevMoves[6] &&
    game->prevMoves[3] == game->prevMoves[11] &&
    game->prevMoves[3] == game->prevMoves[14]
    )
